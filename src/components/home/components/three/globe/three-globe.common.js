...
var GlobeLayerKapsule = Kapsule__default["default"]({
  ...
  stateInit: function stateInit() {
    ...
    var graticulesObj = new THREE$c.LineSegments(new threeGeojsonGeometry.GeoJsonGeometry(d3Geo.geoGraticule10(), GLOBE_RADIUS, 2), new THREE$c.LineBasicMaterial({
      color: 'lightgrey',
      transparent: true,
      opacity: 0.1
    }));
...
var setAttributeFn$1 = new THREE$a.BufferGeometry().setAttribute ? 'setAttribute' : 'addAttribute';
...
var ArcsLayerKapsule = Kapsule__default["default"]({
  ...
  update: function update(state) {
    ...
    var sharedMaterial = new THREE$a.ShaderMaterial(_objectSpread2(_objectSpread2({}, gradientShaders$1), {}, {
      transparent: true,
    ...
    function calcCurve(_ref) {
      ...
      if (altitude) {
        ...
        var curve = _construct(THREE$a.CubicBezierCurve3, _toConsumableArray([startPnt, m1Pnt, m2Pnt, endPnt].map(getVec))); //const mPnt = [...interpolate(0.5), altitude * 2];
...
var applyMatrix4Fn = new THREE$9.BufferGeometry().applyMatrix4 ? 'applyMatrix4' : 'applyMatrix';
var HexBinLayerKapsule = Kapsule__default["default"]({
  ...
  update: function update(state) {
    ...
    var scene = state.hexBinMerge ? new THREE$9.Object3D() : state.scene; // use fake scene if merging hex points
    ...
    if (state.hexBinMerge) {
      // merge points into a single mesh
      var hexPointsGeometry = !hexBins.length ? new THREE$9.BufferGeometry() : BufferGeometryUtils$1.mergeBufferGeometries(hexBins.map(function (d) {
        var obj = d.__threeObj;
        d.__threeObj = undefined; // unbind merged points
        ...
        var topColor = new THREE$9.Color(topColorAccessor(d));
        var sideColor = new THREE$9.Color(sideColorAccessor(d));
        var nVertices = geom.attributes.position.count;
      ...
      var hexPoints = new THREE$9.Mesh(hexPointsGeometry, new THREE$9.MeshBasicMaterial({
        color: 0xffffff,
        vertexColors: true,
      ...
      if (!state.hexBinMerge) {
        ...
        [sideColor, topColor].forEach(function (color) {
          if (!hexMaterials.hasOwnProperty(color)) {
            ...
            hexMaterials[color] = new THREE$9.MeshLambertMaterial({
              color: colorStr2Hex(color),
              transparent: opacity < 1,
              opacity: opacity,
              side: THREE$9.DoubleSide
  ...
  update: function update(state) {
    ...
    threeDigest(singlePolygons, state.scene, {
      ...
      createObj: function createObj() {
        ...
        obj.__defaultSideMaterial = new THREE$8.MeshBasicMaterial({
          side: THREE$8.DoubleSide,
          depthWrite: true
        });
        obj.__defaultCapMaterial = new THREE$8.MeshBasicMaterial({
          side: THREE$8.DoubleSide,
          depthWrite: true
        }); // conic geometry

        obj.add(new THREE$8.Mesh(undefined, [obj.__defaultSideMaterial, // side material
    ...
    threeDigest(state.hexPolygonsData, state.scene, {
      ...
      updateObj: function updateObj(obj, d) {
        ...
        var applyUpdate = function applyUpdate(td) {
          ...
          obj.geometry = !hexBins.length ? new THREE$7.BufferGeometry() : BufferGeometryUtils.mergeBufferGeometries(hexBins.map(function (h) {
            // compute new geojson with relative margin
            var relNum = function relNum(st, end, rat) {
              return st - (st - end) * rat;
            };
...
var PathsLayerKapsule = Kapsule__default["default"]({
  ...
  update: function update(state) {
    ...
    var sharedShaderMaterial = new THREE$5.ShaderMaterial(_objectSpread2(_objectSpread2({}, gradientShaders), {}, {
      transparent: true,
    ...
    threeDigest(state.pathsData, state.scene, {
      ...
      updateObj: function updateObj(group, path) {
        ...
        if (!group.children.length || useFatLine === (group.children[0].type === 'Line')) {
          ...
          var _obj = useFatLine ? new threeFatline.Line2(new threeFatline.LineGeometry(), new threeFatline.LineMaterial()) : new THREE$5.Line(new THREE$5.BufferGeometry(), sharedShaderMaterial.clone() // Separate material instance per object to have dedicated uniforms (but shared shaders)
          );
...
var Globe = Kapsule__default["default"]({
  props: _objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2({
    onGlobeReady: {
      triggerUpdate: false
    },
    rendererSize: {
    ...
    var initGlobe = function initGlobe() {
      if (animateIn) {
        ...
        new TWEEN__default["default"].Tween({
          ...
          k: 1
        }, 600).easing(TWEEN__default["default"].Easing.Quadratic.Out).onUpdate(function (_ref14) {
          var k = _ref14.k;
        ...
        new TWEEN__default["default"].Tween({
          ...
          rot: 0
        }, 1200).easing(TWEEN__default["default"].Easing.Quintic.Out).onUpdate(function (_ref15) {
          var rot = _ref15.rot;
